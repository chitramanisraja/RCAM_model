function [XDOT] = RCAM_model(X,U) %X=State variables and U=Control Variables
...
x9=X(9) %psi
%%
%%Input and Control vector
u1=U(1) %aileron deviation
u2=U(2) %Elevator deflection
u3=U(3) %Rudder deflection
u4=U(4) %throttle1
u5=U(5) %throttle2

%%
%%adfsdf
m=120000; %mass for the aircraft
cbar=6.6; %mean aerodynamic chord
lt=24.8; %distance b/w tail and body
S=260;%wing platform area
St=64;%Tail platform area
Xcg=0.23*cbar;%Cog X position
Ycg=0;%Cog Y position
Zcg=0.1*cbar;%Cog Z position
Xac=0.12*cbar;%Aerodynamic centre x position 
Yac=0;%Aerodynamic centre y position
Zac=0;%Aerodynamic centre z position
...
%%
%calculations
 Va=sqrt(x1^2 + x2^2 + x3^2); %airspeed calculation
 alpha=atan2(x3,x1); %angle of attack
 beta=asin(x2/Va); %Slip Side Angle
...
MAac_b=CMac_b*Q*S*cbar; %moment about aerodynamic centre
